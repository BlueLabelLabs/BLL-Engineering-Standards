...
Technique, not a guideline. Read [Using agents](using-agents.md) first.

## Why it needs its own method

Human code and agent code fail differently, so reviewing them the same way misses things.

Human code is usually wrong because the author misunderstood something. The mistake has a reason behind it, and the reason is often visible: a confused variable name, a comment that argues with itself, an awkward workaround.
...
**Duplication over reuse.** Agents write a new helper more readily than they find the existing one. Invisible in a single diff, obvious across six months of them, which is how a codebase ends up with four date formatters.
...
Five guidelines deserve separate attention. They are the ones we get wrong most often, in both human and agent code, and getting them wrong is a security incident rather than a bug:

| Guideline | The mistake |
| --- | --- |
| [API-007](README.md#api--interfaces-and-contracts) | Accepting a user ID as a parameter instead of deriving it from the token |
| [API-008](README.md#api--interfaces-and-contracts) | Checking that the caller is authenticated but not that they own the target object |
| [API-009](README.md#api--interfaces-and-contracts) | Returning another user's personal fields from a profile endpoint |
| [API-010](README.md#api--interfaces-and-contracts) | Validating an OTP on the client, or returning the code to it |
| [API-011](README.md#api--interfaces-and-contracts) | Comparing the old password client-side on a password change |

These survived from the 2019 standards specifically because they keep happening. An agent writing an endpoint produces something that works correctly for the person testing it and is wide open to everyone else, because "does this work" and "is this authorized" look identical in a happy-path test.

Every one is enforced by review and nothing else. Check them explicitly rather than trusting they came out right.
...
Cite guideline IDs in comments. "This does not satisfy API-008" is more useful than a paragraph, and it gives the author somewhere to go.

If you cannot explain why a section exists, that is the finding. Ask. Do not assume it is fine because the tests pass.

Nothing merges without a second person and green required checks. No exemption for small changes, agent-written changes, or urgent ones ([AGT-003](README.md#agt--working-with-agents), [QUA-004](README.md#qua--quality)).
